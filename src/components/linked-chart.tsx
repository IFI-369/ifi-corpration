...
const DATE_FORMATS = [
  { value: "MMM yyyy", label: "ماه (می 2024)" },
  { value: "QQQ yyyy", label: "فصل (فصل دوم 2024)" },
  { value: "yyyy", label: "سال (2024)" },
  { value: "MM/dd/yyyy", label: "تاریخ آمریکا (12/31/2024)" },
  { value: "dd/MM/yyyy", label: "تاریخ اروپا (31/12/2024)" },
  { value: "yyyy-MM-dd", label: "تاریخ ISO (2024-12-31)" }
] as const;
    ...
    <div className="grid gap-2 text-right">
      <Popover open={open} onOpenChange={setOpen} {...props}>
        ...
        <PopoverContent align="end" className="w-[200px] p-0">
          <Command>
            <CommandInput placeholder="جستجوی فرمت‌های تاریخ" />
            <CommandList>
              <CommandEmpty>هیچ فرمتی یافت نشد</CommandEmpty>
              <CommandGroup>
...
function isValidChartType(type: string): type is ChartType {
  if (!Object.keys(CHART_TYPES).some((t) => t === type)) {
    throw new Error(
      `Invalid chart type: ${type}. Must be one of: ${Object.keys(
        CHART_TYPES
      ).join(", ")}`
    );
  }
...
function isValidDateFormat(format: string): format is DateFormat {
  if (!DATE_FORMATS.some((f) => f.value === format)) {
    throw new Error(
      `Invalid date format: ${format}. Must be one of: ${DATE_FORMATS.map(
        (f) => f.value
      ).join(", ")}`
    );
  }
...
export function LinkedChart<TData extends object>({
  ...
  if (!isValidDataField<TData>(data, dateField)) {
    throw new Error("فیلد تاریخ نامعتبر است");
  }
  if (columns && !isValidDateField(columns, dateField as string))
    throw new Error("Invalid date field configuration");
  if (!isValidDateFormat(dateFormat)) throw new Error("فرمت تاریخ نامعتبر است");
  if (!isValidChartType(chartType)) throw new Error("نوع چارت نامعتبر است");
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <div className="grid gap-3 p-3">
                <span className="text-sm text-right font-medium leading-none">
                  نوع چارت
                </span>
